...
func Generate(outputFile string) error {
	...
	var availableDriversStatements string
	var availableVehiclesStatements string
	wg := sync.WaitGroup{}
	...
	// Phase 2b: Generate available drivers and vehicles
	startAvail := time.Now()
	fmt.Println("Generating available drivers/vehicles...", time.Now())
	driversList := drivers.GenerateDrivers(config.DRIVERS)
	vehiclesList := vehicles.GenerateVehicles(config.VEHICLES)
	availableDrivers := drivers.GenerateAvailableDrivers(driversList, 14) // 14 dni
	availableVehicles := vehicles.GenerateAvailableVehicles(vehiclesList, 14)
	availableDriversStatements = drivers.GenerateAvailableDriversInsertStatements(availableDrivers)
	availableVehiclesStatements = vehicles.GenerateAvailableVehiclesInsertStatements(availableVehicles)
	fmt.Println("done generating available drivers/vehicles", time.Now(), time.Since(startAvail))
	...
	sb.WriteString(availableDriversStatements)
	sb.WriteString(availableVehiclesStatements)
